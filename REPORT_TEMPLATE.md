...
- Calories (kcal) = weight × distance × 1.036 (a general factor for running) [Source](https://www.best-running-tips.com/calories-burned-calculator.html)

**Steps:**

1. Get the numbers the user typed into the input fields.
2. Convert hours to minutes and add them to the minutes field.
3. Check if all necessary values are filled (distance, time, weight). If not, show an alert.
4. Calculate pace, speed, and calories using the formulas.
5. Store the results in variables and display them on the page.
...
I chose these ranges based on the [Compendium of Physical Activities](https://pacompendium.com/running/), which classifies running intensity by speed and pace, and also based on my own experience.
...
- **User leaves input empty?**  
  If distance or weight is empty, `parseFloat` returns `NaN`, so `!distance` or `!weight` becomes true → first alert appears. If hours or minutes are empty, they become 0 because of `|| 0`. So totalMinutes becomes 0 → first alert appears (same as entering zero time).
...
3. Enter your distance (km), time (hours and minutes), and weight (kg)
...
- I noticed the background color changed too fast, so I asked how to make the color transitions smooth.
...
- I picked a hover effect from the options provided and adjusted the timing.
...
From this assignment, I learned how to use variables to store user input and apply conditional logic (if/else) to make decisions based on different results. I finally understood the Input → Process → Output flow, how data enters through HTML, gets processed in javascript and is displayed back to the user. I also realized that programming is about testing, making mistakes, and fixing them step by step. I know I'm still at the beginning and need much more practice, but I'm excited to keep learning through AI, YouTube, other web sites and my own experiments.
